...
function LightPickerCollectionBase() {
	...
	const mountHandler = picker => {
		...
		const isHueSat = colorLightSaturationOidObject?.maxValue === 254;
		...
		const isHueVal = colorLightBrightnessOidObject?.maxValue === 254;
	...
	useEffect(() => {
		...
		const roundedSaturation =
			colorLightSaturationOidObject?.maxValue === 254
				? Math.round((saturationValue / 254) * 100)
				: Math.round(saturationValue);

		const roundedBrightness =
			colorLightBrightnessOidObject?.maxValue === 254
