...
function useData(oid) {
    const { theme, widget, [`${oid}Object`]: oidObject, getPropertyValue } = useContext(CollectionContext);
    ...
    // Hilfsfunktionen
    const formatSize = useCallback(size => (typeof size === 'number' ? `${size}%` : null), []);
    const getDataValue = useCallback((key, ext = '') => widget.data[`${key}${ext}`], [widget.data]);
            ...
            if (oidType === 'number' || oidType === 'string' || oidType === 'boolean' || oidType === 'mixed') {
                for (let i = 1; i <= Number(widget.data.values_count); i++) {
                    ...
                    if (_value !== undefined && _value !== null && /\S/.test(_value)) {
                        ...
                        states.push({
                            value: oidEntry ? oidEntry[0] : oidType === 'number' ? Number(_value) : _value,
                            label: (_alias || (oidEntry ? oidEntry[1] : `${_value}${_unit}`)).replace(
                                /(\r\n|\n|\r)/gm,
                                '',
                            ),
                            ...
                            icon: widget.data[`icon${i}`] || widget.data[`iconSmall${i}`] || widget.data.icon || null,
                            iconSize:
                                typeof widget.data[`iconSize${i}`] === 'number' ? widget.data[`iconSize${i}`] : null,
                            iconWidth:
                            ...
                            iconYOffset:
                                (!!getDataValue('iconYOffset', i) &&
                                    ...
                                    getDataValue('iconYOffset', i)) ||
                                '0px',
                            iconColor: widget.data[`iconColor${i}`] || widget.data.iconColor || null,

                            backgroundColor: widget.data.backgroundColor || backgroundStyles.backgroundColor,
                            ...
                            frameBackgroundColor: widget.data.frameBackgroundColor || backgroundStyles.backgroundColor,
                            frameBackgroundColorActive: getDataValue('frameBackgroundColor', i),

                            frameBackground: widget.data.frameBackground || backgroundStyles.background,
    ...
    const data = useMemo(() => {
        const getStyleData = (ext = '') => ({
            textColor: widget.data.textColor || textStyles.color || theme.palette.primary.main,
            ...
            footer: (widget.data.footerActive || getDataValue('footer', ext) || widget.data.footer || '').replace(
                /(\r\n|\n|\r)/gm,
                '',
            ),
            footerSize:
            ...
            valueSizeActive:
                typeof getDataValue('valueSize', ext) === 'number' && formatSize(getDataValue('valueSize', ext)),

            icon: !widget.data.noIcon && (widget.data.icon || widget.data.iconSmall),
            iconActive: !widget.data.noIcon && (getDataValue('icon', ext) || getDataValue('iconSmall', ext)),
            iconSize:
                (typeof widget.data.iconSize === 'number' && `calc(24px * ${widget.data.iconSize} / 100)`) || '24px',
            iconSizeActive: getDataValue('iconSize', ext) && `calc(24px * ${getDataValue('iconSize', ext)} / 100)`,
            ...
            iconHoverActive: getDataValue('iconHover', ext) && `${getDataValue('iconHover', ext)}%`,
